...
//登録用スプレッドシートID
const GSS_USERS_SHEETID =
  PropertiesService.getScriptProperties().getProperty('GSS_USERS_SHEETID')
const GSS_USERS_SHEATNAME = PropertiesService.getScriptProperties().getProperty(
  'GSS_USERS_SHEATNAME',
)
//Trello API関連
const TRELLO_API_URL =
  PropertiesService.getScriptProperties().getProperty('TRELLO_API_URL')
const TRELLO_USER_ID =
  PropertiesService.getScriptProperties().getProperty('TRELLO_USER_ID')
const TRELLO_KEY =
  PropertiesService.getScriptProperties().getProperty('TRELLO_KEY')
//Discord API関連
const DISCORD_WEBHOOK_URL = PropertiesService.getScriptProperties().getProperty(
  'DISCORD_WEBHOOK_URL',
)
//その他
const FILTER_TARGET_WORDS = PropertiesService.getScriptProperties()
  .getProperty('FILTER_TARGET_WORDS')
  .split(',')
const BORDER_CLOSETASK =
  PropertiesService.getScriptProperties().getProperty('BORDER_CLOSETASK')
//ユウカのセリフ
const MESSAGE_TO_CLOSETASK =
  '先生！以下のタスクが期限間近です。後回しにせず、早く取り掛かってくださいね。'
const MESSAGE_TO_EXPIREDTASK =
  '先生？以下のタスクはもう期限を過ぎていますよ！全く、しっかりしてください！'
...
function getUsersFromSheet() {
  ...
  while (hasData) {
    const rowData = sheet
      .getRange(row, startColumn, 1, numColumns)
      .getValues()[0]
    ...
    if (rowData.join('') !== '') {
      ...
      const sameUserIndex = users.findIndex(function (value) {
        return value.mailAddr == _mailAddr
      })
      if (sameUserIndex == -1) {
        users.push({
...
function createTasks(cards) {
  ...
  for (let card of cards) {
    if (card.due != null && !card.dueComplete) {
      const timeLeft =
        Utilities.parseDate(card.due, 'UTC', "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") -
        nowDateTime
      resultTasks.push({
  ...
  for (let task of tasks) {
    if (task.timeLeft < BORDER_CLOSETASK && task.timeLeft > 0) {
      taskString = taskString.concat(
        `${task.name}: 残り${Math.round(
          task.timeLeft / ONEHOUR_BY_MILISEC,
        ).toString()}時間\n`,
      )
    }
  }
...
function postExpiredTasks(tasks, user) {
  ...
  for (let task of tasks) {
    if (task.timeLeft < 0) {
      taskString = taskString.concat(
        `${task.name}: 期限を${Math.abs(
          Math.round(task.timeLeft / ONEHOUR_BY_MILISEC),
        ).toString()}時間超過\n`,
      )
    }
  }
